                ...
                <div className="grid lg:grid-cols-12 gap-10">
                    ...
                    <div className="lg:col-span-3 space-y-4">
                        <div className="flex items-center gap-2 mb-6 ml-1">
                            ...
                            <h2 className="text-[10px] font-bold text-zinc-500 dark:text-gray-500 uppercase tracking-[0.2em] font-mono">
                                Assigned Queue
                            </h2>
                        </div>

                        {loadingQueue ? (
                            <div className="p-12 text-center border-1.5 border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0f1014] rounded-3xl shadow-sm">
                                <Loader className="animate-spin text-purple-500 dark:text-purple-400 mx-auto" size={24} />
                            </div>
                        ) : papers.length === 0 ? (
                            <div className="p-12 text-center border-1.5 border-dashed border-zinc-200 dark:border-white/10 bg-white dark:bg-transparent rounded-3xl text-zinc-400 dark:text-gray-600 text-xs italic font-mono">
                                No assigned audits.
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {papers.map(paper => (
                                    <div
                                        ...
                                        className={`p-6 cursor-pointer transition-all border-1.5 rounded-2xl text-left hover:translate-y-[-2px] ${
                                            selectedPaperId === paper.id
                                                ? 'border-purple-400 dark:border-purple-500 bg-purple-500/5 dark:bg-purple-950/20 shadow-lg shadow-purple-500/5'
                                                : 'border-zinc-200 dark:border-white/5 bg-white dark:bg-[#0f1014]/60 hover:border-zinc-300 dark:hover:border-white/20 shadow-sm hover:shadow-md'
                                        }`}
                                    >
                                        <div className="flex justify-between items-start gap-2 mb-3">
                                            <span className="text-[9px] font-mono text-purple-500 dark:text-purple-400 font-bold">
                                                #{String(paper.id).slice(0, 8)}
                                            </span>
                                            <span className={`text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full font-mono ${statusBadge(paper.status)}`}>
                                                {paper.status}
                                            </span>
                                        </div>
                                        <h3 className="font-bold text-xs text-zinc-800 dark:text-gray-200 line-clamp-2 leading-snug">
                                            {paper.title.replace('Audit Request: ', '')}
                                        </h3>
                                        <p className="text-[10px] text-zinc-555 dark:text-gray-500 mt-2.5 truncate font-mono">
                    ...
                    <div className="lg:col-span-9">
                        {selectedPaperId ? (
                            loadingContent ? (
                                <div className="flex flex-col items-center justify-center min-h-[550px] border-1.5 border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0f1014] rounded-3xl shadow-xl">
                                    ...
                                    <div className="lg:col-span-7 flex flex-col h-[78vh] border-1.5 border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0f1014] rounded-3xl overflow-hidden shadow-2xl">
                                        <header className="px-6 py-5 border-b border-zinc-100 dark:border-white/5 flex justify-between items-center bg-zinc-50 dark:bg-black/40">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-zinc-600 dark:text-gray-400 uppercase tracking-wider font-mono">
                                        ...
                                        <div className="flex-1 overflow-y-auto p-6" ref={readerContainerRef}>
                                            {activePaneTab === 'pdf' && paperContent.pdf_url && paperContent.source !== 'pitch' ? (
                                                <iframe
                                                    ...
                                                    className="w-full h-full border-0 rounded-2xl bg-zinc-105 dark:bg-black"
                                                    title="PDF Document"
                                                />
                                            ) : (
                                    ...
                                    <div className="lg:col-span-5 flex flex-col h-[78vh] overflow-y-auto space-y-6">
                                        ...
                                        {selectedText && (
                                            <div className="p-6 sm:p-8 border border-purple-250 dark:border-purple-500/20 bg-purple-50/50 dark:bg-purple-950/5 rounded-3xl shadow-md text-left animate-in slide-in-from-top-4 duration-300">
                                                <div className="flex justify-between items-center mb-4">
                                                    ...
                                                    <span className="text-[10px] text-zinc-400 dark:text-gray-500 font-mono font-bold">
                                                        Page {selectedPageNum}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex gap-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-4 py-3 border border-zinc-200 dark:border-white/10 bg-white dark:bg-transparent hover:bg-zinc-50 dark:hover:bg-white/5 text-zinc-500 dark:text-gray-400 text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer"
                                                    >
                                                        Cancel
                                                    </button>
                                                </div>
                                        ...
                                        <div className="p-6 sm:p-8 border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0f1014] rounded-3xl shadow-lg text-left">
                                            ...
                                            {paperContent.annotations?.length === 0 ? (
                                                <div className="py-8 text-center text-zinc-450 dark:text-gray-600 text-xs italic font-mono">
                                                    ...
                                                    {paperContent.annotations.map(anno => (
                                                        <div key={anno.id} className="p-4 bg-zinc-50 dark:bg-black/40 border border-zinc-150 dark:border-white/5 rounded-2xl flex gap-3 items-start justify-between group">
                                                            <div className="flex-1 text-left min-w-0">
                                                                <div className="text-[9px] font-mono text-zinc-400 dark:text-gray-500 font-bold uppercase mb-1.5">
                                                                    Page {anno.page}
                                                                </div>
                                                            ...
                                                            <button
                                                                ...
                                                                className="text-zinc-350 dark:text-zinc-600 hover:text-red-500 dark:hover:text-red-400 transition-colors p-1 rounded-lg ml-1 cursor-pointer shrink-0"
                                                            >
                                                                <Trash2 size={13} />
                                                            </button>
                                                        </div>
                                        ...
                                        {/* Assessment Form */}
                                        <div className="p-6 sm:p-8 border-1.5 border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0f1014] rounded-3xl shadow-xl text-left flex-1 flex flex-col justify-between">
                                            <div>
                            ...
                            <div className="h-full min-h-[550px] border border-dashed border-zinc-200 dark:border-white/10 bg-white dark:bg-transparent flex flex-col items-center justify-center text-center p-16 rounded-3xl shadow-sm">
                                <div className="p-6 rounded-2xl bg-purple-50 dark:bg-purple-900/10 mb-6 border border-purple-100 dark:border-purple-500/10">
                                    <ShieldCheck size={44} className="text-purple-400 dark:text-purple-500/60" />
                                </div>
                                <h3 className="text-base font-bold text-zinc-800 dark:text-gray-300 uppercase tracking-wide font-mono">Select a paper to review</h3>
                                <p className="max-w-md text-sm mt-3 text-zinc-405 dark:text-gray-500 leading-relaxed font-light">
                                    Choose an assigned paper from the queue on the left to load the reviewer workspace, add Claim commentary, and finalize the evaluation notes.
                                </p>
                            </div>
                        )}
                    </div>
